...
size_t get_file_size_fs(const FileSource &file) {
...
SecByteBlock read_key(char *keyf) {
    try {
        ...
        if (get_file_size_fs(fsource) != ENC_KEY_SIZE + HASH_KEY_SIZE) {
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 8 && strcmp(argv[1], "-k") == 0 && strcmp(argv[3], "-e") == 0 &&
        ...
        try {
            ...
            size_t file_size = get_file_size_fs(fsource);
            size_t width =
        ...
        try {
            std::ifstream infile(argv[5], std::ios::in | std::ios::binary);
            if (!infile.is_open()) {
                error_exit("[main] infile");
            }
